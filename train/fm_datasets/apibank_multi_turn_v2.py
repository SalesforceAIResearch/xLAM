...
class SFTAPIBankMultiTurnV2(SFTFoundationModelDataBase):

    def __init__(self, tokenizer, args):
        super().__init__(tokenizer, args)
        self.name = "SFTAPIBankMultiTurn_v2"
    ...
    def create_datasets(self, return_type="constant_length", seed=None):
        # in total 4902 rows of records
        train_data = load_dataset(
            os.path.join(self.loader_dir, "unified_dataset.py"),
            split="train",
            data_dir=os.path.join(self.data_save_dir, "apibank"),
            max_context_length=self.args.seq_length,
            cache_dir=os.path.join(self.data_save_dir, "monica_cached", "apibank"),
        ...
        train_data = train_data.shuffle(seed=seed, buffer_size=5000)
