...
class SFTFoundationTrainer(Trainer):
    r"""
    Class definition of the Supervised Finetuning Trainer (SFT Trainer) modified from trl codebase.
    ...
    def __init__(
        ...
        if not packing and data_collator is None:
    ...
    def _prepare_dataset(
        ...
        # post process outside of the trainer, do nothing
        if isinstance(dataset, (ConstantLengthDataset, PromptAnswerDataset)):
            return dataset
        ...
        if not packing:
            if dataset_text_field is None and formatting_func is None:
                warnings.warn(
                    "You passed `packing=False` to the SFTTrainer, "
                    "but you didn't pass a `dataset_text_field` or `formatting_func` argument."
                )
